...
impl AuthOperations {
    ...
    ///
    /// This method works with token and refresh. The document is set TTL to `REFRESH_MAX_AGE`,
    /// but the token has a shorter life span of `TOKEN_MAX_AGE`.When `jsonwebtoken` invalidates the token,
    /// the expired token won't reach here.
    ///
    /// But in some cases, better be safe than sorry.
    ...
    async fn refetch(&mut self, claims: &KeyClaims) -> Result<bool, AuthOperationError> {
